...
In this package, all non-parametric models rely on Dagnelie’s equations. These equations are practical because they allow volume estimation for a wide range of tree species.

- *The single-entry method* provides the simplest way to estimate tree volume, requiring only `c130`.
- *The single-entry graduated method* offers improved accuracy for pure even-aged stands.
- *The two-entry method* is the most precise model, but it requires measuring `htot`, which may be difficult in large stands. It should therefore be used mainly for high-value trees.
...
across a wide range of tree dimensions. Based on geometrical parameter they are 
more precise to evaluate coniferous trees (because of their shape homogenity)
...
# Biomass/carbon/CO2 storage module
...
## Biomass/carbon/CO2 calculator :`biomass_calc()`

This function include 2 several method:

### 1. CNIFEB method

CNIFEB (Compagnie Nationale des Ingenieurs et Experts Forestiers et des Experts Bois)
...
# Plot + export CSV
res <- plot_by_class(data, volume_col = "dagnelie_vc22_1", output = filepath)
